...
**右侧排行跟随左侧的时间窗口**，面板标题会显示当前统计的是哪一段
（`2026-08-12` / `2026-08-09 起那周` / `近 1 年`）。范围按钮（3 个月 / 6 个月 /
1 年）同时限定数据池。所以「今天我在哪个项目烧得多」和「这一年总账」是同一个
控件切出来的。

**按项目以项目为组**，一个项目一行，条形按来源分段堆叠——同一个项目常常横跨多个
渠道，拆成多行就看不出它总共烧了多少。悬停任意一段显示该来源的具体量和占比
（`ai · CC-Switch 10.6M · 占 61%`）。排行区放不下就滚动，不再按高度裁行数。
