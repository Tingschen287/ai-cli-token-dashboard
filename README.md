...
- CPU 占用 = `/proc/stat` 相邻两轮差值；内存 = `/proc/meminfo`；磁盘 = WSL 根
  分区 + Windows 的 C: / D:（三块并排小环，各自悬停看明细）；GPU 走 WSL 直通的
  `nvidia-smi`（利用率大环，显存 + 温度在副行，悬停看显卡名）。
- **CPU 温度拿不到**：WSL2 是虚拟机，没有硬件传感器入口；GPU 温度跟着
  `nvidia-smi` 有了。CPU 环副行显示核心数。
...
**右侧三块卡片竖排三等分：By Model、Performance、By Project**。Performance 是
本机性能实时镜像（CPU / 内存 / GPU / 磁盘四环），下面细说。
